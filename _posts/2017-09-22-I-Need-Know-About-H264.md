...
_以前看书后分享的只是零碎的笔记，没敢发布博客，这一篇我力求根据自己的理解，把上述两个核心问题描述清楚，细节内容篇幅有限，就不做展开了。感兴趣的朋友可以阅读原书，当然，最正宗的资料莫过于 [H.264 SPEC](https://www.itu.int/rec/T-REC-H.264) 了。_
...
+ `640*480*4*8*30 = 281.25 Mbps`（_`宽*高*像素字节数*字节比特数*帧数`_）
...
+ `skip_run` 是不编码的宏块，因为我们发现有些宏块编码之后没有什么压缩收益，还不如不编码直接传输；
